...
DriveBase::DriveBase(int l1, int l2, int l3, int l4,
	...
	if (two_speed) { //StartLow()
		...
		actual_max_y_rpm = MAX_Y_FPS_AUTON;
...
void DriveBase::TeleopWCDrive(Joystick *JoyThrottle, //finds targets for the Controller()
  ...
  if (!hybrid) { //normal teleop wc wheel input
...
void DriveBase::GenerateVisionProfile(double dist_to_target, double yaw_to_target, double exit_angle) {
  ...
  frc::SmartDashboard::PutNumber("V exit angle", exit_angle);
	...
	double x_dist = dist_to_target * sin(std::abs(yaw_to_target));
  double y_dist = dist_to_target * cos(std::abs(yaw_to_target));
  ...
  frc::SmartDashboard::PutNumber("V y dist", y_dist);
	...
	p1 = {0.0, 0.0, 0.0};
	p2 = {y_dist, x_dist, 0}; //y, x, exit angle
  ...
  pathfinder_prepare(points, POINT_LENGTH, FIT_HERMITE_CUBIC,
	PATHFINDER_SAMPLES_FAST, 0.02, MAX_Y_FPS_AUTON, MAX_ACC_VIS, MAX_JERK_VIS, &candidate);
...
//	}
  ...
  if (frc::RobotState::IsAutonomous()) { //only want the feedforward based off the motion profile during autonomous. The root generated ones (in the if() statement) //should already be 0 during auton because we send 0 as refs
    ...
    if (ref_right < 0.0) {
      k_f_right_vel = 1.0 / MAX_VEL_VIS;
    } else {
      k_f_right_vel = 1.0 / MAX_VEL_VIS;
    }

    if (ref_left < 0.0) {
      k_f_left_vel = 1.0 / MAX_VEL_VIS;
    } else {
      k_f_left_vel = 1.0 / MAX_VEL_VIS;
    }
...
void DriveBase::RunTeleopDrive(Joystick *JoyThrottle,
	Joystick *JoyWheel, bool is_regular, bool is_vision, bool is_rotation) {
      ...
      frc::SmartDashboard::PutString("DRIVE", "reg");
			...
			case VISION_PF:
      frc::SmartDashboard::PutString("DRIVE", "vision pf");
