...
void ArmMotionProfiler::SetFinalGoalArm(double goal) {

	final_goal_a = goal;

}

void ArmMotionProfiler::SetInitPosArm(double position_init) { //at every new whole profile
...
void ArmMotionProfiler::SetMaxAccArm(double max_acc) {
...
std::vector<std::vector<double>> ArmMotionProfiler::GetNextRefArm() { //used by both elevator and intake
	...
	frc::SmartDashboard::PutNumber("FINAL GOAL", final_goal_a);
	...
	while (counter < interval) {
		...
		frc::SmartDashboard::PutNumber("profile velocity", vel);
		frc::SmartDashboard::PutNumber("prfile position", pos);
		counter++;
	}

	frc::SmartDashboard::PutNumber("ramp time", ramp_time);
	frc::SmartDashboard::PutNumber("ramp distance", ramp_dis);
	frc::SmartDashboard::PutNumber("acc", acc);
...
std::vector<std::vector<double> > ArmMotionProfiler::CreateProfile1DArm(
	...
	for (int a = 0; a < length_waypoint; a++) { //will make a profile between each waypoint
...
double ArmMotionProfiler::FindArmAngle(std::vector<double> p1,
